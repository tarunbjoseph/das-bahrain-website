...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 w-full">
      ...
      <div className="bg-gradient-to-r from-emerald-900 via-das-900 to-das-950 border-b border-white/5 py-1.5 px-4 text-xs text-slate-300">
      ...
      {/* Main Navbar */}
      <nav className="glass-panel border-b border-white/10 px-4 lg:px-8 py-3 transition-all">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          >
            <div className="relative w-12 h-12 rounded-full overflow-hidden bg-black p-0.5 border-2 border-emerald-500/60 shadow-lg shadow-emerald-900/30 group-hover:border-emerald-400 transition-all flex items-center justify-center">
              <img 
                src="/das-logo.png" 
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-1.5 py-0.5 rounded font-bold uppercase tracking-wider hidden sm:inline-block">
                  Licensed
                </span>
              </div>
          ...
          <div className="hidden md:flex flex-1 max-w-md mx-4">
            <div className="relative w-full">
              <input
                ...
                className="w-full bg-das-900/80 border border-white/15 focus:border-emerald-500 rounded-full py-2 px-10 text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 transition-all"
              />
              <Search className={`absolute top-2.5 w-4 h-4 text-slate-400 ${isRtl ? 'right-3.5' : 'left-3.5'}`} />
              {searchQuery && (
                <button 
                  onClick={() => onSearchChange('')}
                  className={`absolute top-2.5 text-xs text-slate-400 hover:text-white ${isRtl ? 'left-3.5' : 'right-3.5'}`}
                >
                  <X className="w-4 h-4" />
                </button>
              )}
          ...
          <div className="hidden lg:flex items-center gap-6 text-sm font-medium text-slate-300">
            ...
            <button 
              ...
              className="flex items-center gap-1.5 text-emerald-400 hover:text-emerald-300 transition-colors font-semibold py-1 px-2.5 rounded-lg bg-emerald-950/50 border border-emerald-500/30"
            >
          ...
          {/* Action Controls: Language Toggle & Cart Button */}
        ...
        {searchOpen && (
          <div className="md:hidden mt-3 pt-3 border-t border-white/10 animate-fadeIn">
            <div className="relative w-full">
              <input
                type="text"
                placeholder={t.searchPlaceholder}
                value={searchQuery}
                onChange={(e) => onSearchChange(e.target.value)}
                className="w-full bg-das-900 border border-white/20 focus:border-emerald-500 rounded-xl py-2 px-10 text-sm text-white focus:outline-none"
              />
              <Search className={`absolute top-2.5 w-4 h-4 text-slate-400 ${isRtl ? 'right-3.5' : 'left-3.5'}`} />
            </div>
          </div>
        ...
        {/* Mobile Drawer Menu */}
        {mobileMenuOpen && (
          <div className="lg:hidden mt-3 pt-3 border-t border-white/10 space-y-2 pb-2 animate-fadeIn">
            <button 
              onClick={() => handleNavClick('products')}
              className="w-full text-left rtl:text-right px-3 py-2 text-sm font-medium text-slate-200 hover:bg-white/5 rounded-lg"
            >
              {t.shop}
            </button>
            <button 
              onClick={() => handleNavClick('b2b')}
              className="w-full text-left rtl:text-right px-3 py-2 text-sm font-semibold text-emerald-400 bg-emerald-950/40 border border-emerald-500/20 rounded-lg flex items-center justify-between"
            >
              <span>{t.b2bWholesale}</span>
            ...
            <div className="pt-2 border-t border-white/10 flex flex-col gap-2">
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 py-2 px-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold"
              >
